...
def compute_f_alpha(
    ...
    normalization_factor = f_alpha_rgi(np.stack(
        [
            30 * np.ones(spectrum_size),
    ...
    if np.isfinite(incidence_angle):
        if limb_darkening == "Lommel-Seeliger":
            if np.cos(incidence_angle * (np.pi/180)) < 0.05:
                LDF = XL(30, 0, 30) / XL(np.acos(0.05), emission_angle,
                                         phase_angle)
            elif np.cos(incidence_angle * (np.pi/180)) > 0.05:
                LDF = XL(30, 0, 30) / XL(incidence_angle, emission_angle,
                                         phase_angle)
            else:
                raise ValueError(
        ...
        photometric_coefficients = np.full(
            f_alpha.size, np.nan
        )
    else:
        raise ValueError(
...
def compute_limb_darkening_correction_factor(
    ...
    limb_darkening_normalization = ldf_dispatcher[method](
        30, 0, 30
    )
